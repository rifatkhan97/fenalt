import type { Metadata } from "next";
import Link from "next/link";
import { ArrowRight, ShieldCheck, Layers, Sparkles, RefreshCw } from "lucide-react";

export const metadata: Metadata = {
  title: "Fabric Sourcing Bangladesh | Fenalt",
  description:
    "Direct fabric sourcing services in Dhaka, Bangladesh. Access circular deadstock ('Jhoot') fabrics, combed cotton knits, French Terry, and custom mill textiles.",
  alternates: {
    canonical: "https://www.fenalt.com/fabric-sourcing-bangladesh",
  },
  openGraph: {
    title: "Fabric Sourcing Bangladesh | Fenalt",
    description:
      "Direct fabric sourcing services in Dhaka, Bangladesh. Access circular deadstock ('Jhoot') fabrics, combed cotton knits, French Terry, and custom mill textiles.",
    url: "https://www.fenalt.com/fabric-sourcing-bangladesh",
  },
};

export default function FabricSourcingBangladeshPage() {
  const serviceJsonLd = {
    "@context": "https://schema.org",
    "@type": "Service",
    "name": "Fabric Sourcing Bangladesh",
    "provider": {
      "@type": "Organization",
      "name": "Fenalt",
      "url": "https://www.fenalt.com",
    },
    "serviceType": "Textile Sourcing Bangladesh, Deadstock Fabric Sourcing, Combed Cotton Knit Procurement",
    "areaServed": ["United States", "United Kingdom", "European Union", "Australia", "Canada"],
    "description":
      "Dhaka-based textile and fabric sourcing for fashion brands. Access high-grade circular deadstock cotton knits, heavy fleece, and custom woven textiles without high mill minimums.",
  };

  const breadcrumbsJsonLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    "itemListElement": [
      {
        "@type": "ListItem",
        "position": 1,
        "name": "Home",
        "item": "https://www.fenalt.com",
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": "Fabric Sourcing Bangladesh",
        "item": "https://www.fenalt.com/fabric-sourcing-bangladesh",
      },
    ],
  };

  const faqs = [
    {
      q: "What is circular deadstock ('Jhoot') fabric in Bangladesh?",
      a: "Circular deadstock (locally known as 'Jhoot') consists of high-quality leftover fabric rolls from major export mill runs in Dhaka. Sourcing deadstock allows small brands to access premium textiles without fabric mill MOQs.",
    },
    {
      q: "What fabric weights can Fenalt source in Dhaka?",
      a: "We source 180gsm–300gsm combed ring-spun cotton knits, 380gsm–480gsm French Terry fleece, ribbing, twills, and custom woven blends.",
    },
  ];

  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "mainEntity": faqs.map((faq) => ({
      "@type": "Question",
      "name": faq.q,
      "acceptedAnswer": {
        "@type": "Answer",
        "text": faq.a,
      },
    })),
  };

  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify([serviceJsonLd, breadcrumbsJsonLd, faqJsonLd]) }}
      />

      <section className="pt-32 pb-20 lg:pt-44 lg:pb-28 bg-[#F2EFE9] relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#C8A882] mb-6">
            Textile Procurement
          </p>
          <h1 className="font-display text-5xl sm:text-6xl lg:text-8xl font-light text-[#1A1A1A] leading-[1.05] max-w-5xl mb-8">
            Fabric Sourcing Services in{" "}
            <span className="italic font-medium text-[#2D5016]">Dhaka, Bangladesh.</span>
          </h1>
          <p className="text-lg lg:text-xl text-[#6B6560] leading-relaxed max-w-2xl mb-12">
            Unlock premium textiles directly in Dhaka. We source circular deadstock cotton knits, heavyweight French Terry fleece, and custom woven textiles without forcing massive mill minimums.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <Link
              href="/intake"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#1A1A1A] text-[#FAF9F6] text-sm font-semibold tracking-wide hover:bg-[#2D5016] transition-colors duration-300"
            >
              Request Fabric Swatches <ArrowRight size={16} />
            </Link>
            <Link
              href="/sustainable-textile-sourcing"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 border border-[#1A1A1A] text-[#1A1A1A] text-sm font-semibold tracking-wide hover:bg-[#1A1A1A] hover:text-[#FAF9F6] transition-colors duration-300"
            >
              Sustainable Sourcing
            </Link>
          </div>
        </div>
      </section>

      <section className="py-20 lg:py-28 bg-[#FAF9F6]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#C8A882] mb-4">
              Dhaka Textile Ecosystem
            </p>
            <h2 className="font-display text-3xl lg:text-5xl font-light text-[#1A1A1A] mb-8">
              Navigating Bangladesh Textile Markets
            </h2>
            <div className="space-y-6 text-[#6B6560] text-base lg:text-lg leading-relaxed">
              <p>
                Bangladesh is home to one of the world&apos;s most sophisticated textile ecosystems. However, small brands often encounter massive mill minimums (1,000kg+ per dye lot) when trying to order custom fabrics directly.
              </p>
              <p>
                <strong>Fenalt unlocks small-batch fabric sourcing:</strong> Our Dhaka sourcing team accesses high-grade circular deadstock (&lsquo;Jhoot&rsquo;) fabric rolls from major export mill overruns, enabling premium knit production starting at 50 units.
              </p>
            </div>
          </div>
        </div>
      </section>

      <section className="py-24 lg:py-32 bg-[#F2EFE9]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="mb-16">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#C8A882] mb-4">
              Fabric Categories
            </p>
            <h2 className="font-display text-4xl lg:text-6xl font-light text-[#1A1A1A]">
              Textile Capabilities
            </h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {[
              {
                icon: Layers,
                title: "Combed Cotton Knits",
                tag: "180gsm – 300gsm",
                body: "Single jersey, interlock, and heavy ribbing in combed ring-spun cottons for t-shirts and longsleeves.",
              },
              {
                icon: Sparkles,
                title: "Heavyweight Fleece",
                tag: "380gsm – 480gsm",
                body: "100% cotton French Terry and brushed fleece for luxury hoodies and sweatpants.",
              },
              {
                icon: RefreshCw,
                title: "Jhoot Deadstock Fabrics",
                tag: "Zero Mill MOQ",
                body: "Leftover export mill fabric rolls, lowering material cost and environmental footprint.",
              },
            ].map((item, i) => {
              const Icon = item.icon;
              return (
                <div key={i} className="bg-[#FAF9F6] border border-[#E5DDD3] p-8 lg:p-10 flex flex-col justify-between">
                  <div>
                    <div className="w-12 h-12 border border-[#C8A882] flex items-center justify-center text-[#C8A882] mb-8">
                      <Icon size={20} strokeWidth={1.5} />
                    </div>
                    <span className="inline-block px-3 py-1 bg-[#F2EFE9] text-[#2D5016] text-xs font-semibold uppercase tracking-wider mb-4">
                      {item.tag}
                    </span>
                    <h3 className="font-display text-2xl font-medium text-[#1A1A1A] mb-4">{item.title}</h3>
                    <p className="text-sm text-[#6B6560] leading-relaxed">{item.body}</p>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </section>

      <section className="py-24 lg:py-32 bg-[#1A1A1A] text-[#FAF9F6]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#C8A882] mb-6">
              Quality Assurance
            </p>
            <h2 className="font-display text-4xl lg:text-6xl font-light text-[#FAF9F6] mb-8">
              Physical Fabric Inspection Before Cutting
            </h2>
            <p className="text-lg text-[#E5DDD3] leading-relaxed font-light mb-8">
              We inspect GSM weight, color fastness, shrinkage tolerance, and fabric hand-feel on the floor in Dhaka before cutting begins.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12">
            {[
              { title: "GSM Weight Testing", desc: "Verifying exact fabric density prior to pattern cutting." },
              { title: "Shrinkage Verification", desc: "Pre-wash testing to ensure measurement tolerance stability." },
              { title: "Color Fastness Audit", desc: "Testing dye stability against washing and crocking." },
            ].map((qc, i) => (
              <div key={i} className="p-6 bg-[#2A2A2A] border border-[#3A3A3A]">
                <ShieldCheck size={20} className="text-[#C8A882] mb-4" />
                <h3 className="font-display text-lg font-medium text-[#FAF9F6] mb-2">{qc.title}</h3>
                <p className="text-xs text-[#E5DDD3] leading-relaxed">{qc.desc}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="py-24 lg:py-32 bg-[#FAF9F6]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          <div className="mb-16">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#C8A882] mb-4">
              Questions
            </p>
            <h2 className="font-display text-4xl lg:text-6xl font-light text-[#1A1A1A]">
              Fabric Sourcing FAQ
            </h2>
          </div>

          <div className="space-y-8">
            {faqs.map((faq, i) => (
              <div key={i} className="p-8 bg-[#F2EFE9] border border-[#E5DDD3]">
                <h3 className="font-display text-xl font-medium text-[#1A1A1A] mb-3">
                  {faq.q}
                </h3>
                <p className="text-sm text-[#6B6560] leading-relaxed">
                  {faq.a}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="py-20 lg:py-24 bg-[#FAF9F6] border-t border-[#E5DDD3]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <h2 className="font-display text-4xl lg:text-5xl font-light text-[#1A1A1A] mb-6">
            Source custom textiles for your garments
          </h2>
          <p className="text-[#6B6560] leading-relaxed text-base lg:text-lg mb-10 max-w-xl mx-auto">
            Submit your fabric specifications or target GSM for physical swatch recommendations within 24–48 hours.
          </p>
          <Link
            href="/intake"
            className="inline-flex items-center justify-center gap-2 px-10 py-4 bg-[#1A1A1A] text-[#FAF9F6] text-sm font-semibold tracking-wide hover:bg-[#2D5016] transition-colors duration-300"
          >
            Start Fabric Sourcing <ArrowRight size={16} />
          </Link>
        </div>
      </section>
    </>
  );
}
